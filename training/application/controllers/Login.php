	...
	function validation()
	{
	      ...
	      if($this->form_validation->run())
	      {
	         ...
	         if($result == '')
	         {
				...
				elseif ($this->session->userdata('formateur_login')) {

					$id=$this->session->userdata('formateur_login');
				    $cours = $this->db->get_where('online', array(
						    	'id_user'	=>	$id
						    ));
			        ...
			        if ($cours->num_rows() > 0) {
			        	...
			        	$insert = array(
							'id_user'	=>	$this->session->userdata('formateur_login')
						);
						$this->crud_model->insert_online($insert);
			        }

					redirect('formateur/dashbord');
